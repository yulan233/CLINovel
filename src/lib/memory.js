...
export async function rebuildMemory(rootDir) {
  ...
  return rebuildMemoryAggregates(rootDir);
}
...
export async function archiveMemory(rootDir) {
  ...
  const archivedDocs = docs.slice(0, -3);
...
export async function searchMemory(rootDir, filters = {}) {
  ...
  if (filters.thread) {
    ...
    chapters = chapters.filter((item) => (item.keyThreads || []).some((name) => canonicalMemoryKey(name).includes(query)));
  }
...
export async function rebuildMemoryAggregates(rootDir) {
  ...
  docs.sort((a, b) => a.chapterId.localeCompare(b.chapterId));
  const recentDocs = docs.slice(-3);
  const olderDocs = docs.slice(0, -3);
...
function applyContextBudget(sections, budget, options = {}) {
  ...
  for (const section of sorted) {
    const fullCost = estimateTokenCost(section.text);
    ...
    const compressedCost = estimateTokenCost(compressedText);
